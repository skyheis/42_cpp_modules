...
/*   Updated: 2023/06/15 18:32:32 by ggiannit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::executeForm(AForm const &frm) {
	try {
		frm.execute(*this);
		std::cout << this->_name << " executes " << frm.getName() << std::endl;
	}
	catch (std::exception &e) {
		std::cerr << this->_name << " cannot execute " << frm.getName() << " because " << e.what() << std::endl;
	}
}
